...
public class CompoundTag_v1_19_R1 extends CompoundTag {
    public CompoundTag_v1_19_R1(Map<String, Tag> value) {
        super(value);
    }

    public static CompoundTag fromNMSTag(NBTTagCompound tag) {
        HashMap<String, Tag> tags = new HashMap<String, Tag>();
        for (String key : tag.d()) {
            NBTTagList list;
            NBTBase base = tag.c(key);
            if (base instanceof NBTTagInt) {
                tags.put(key, new IntTag(((NBTTagInt) base).f()));
                continue;
            }
            if (base instanceof NBTTagByte) {
                tags.put(key, new ByteTag(((NBTTagByte) base).h()));
                continue;
            }
            if (base instanceof NBTTagFloat) {
                tags.put(key, new FloatTag(((NBTTagFloat) base).j()));
                continue;
            }
            if (base instanceof NBTTagDouble) {
                tags.put(key, new DoubleTag(((NBTTagDouble) base).i()));
                continue;
            }
            if (base instanceof NBTTagByteArray) {
                tags.put(key, new ByteArrayTag(((NBTTagByteArray) base).d()));
                continue;
            }
            if (base instanceof NBTTagIntArray) {
                tags.put(key, new IntArrayTag(((NBTTagIntArray) base).f()));
                continue;
            }
            if (base instanceof NBTTagCompound) {
                tags.put(key, CompoundTag_v1_19_R1.fromNMSTag((NBTTagCompound) base));
                continue;
            }
            ...
            if (base instanceof NBTTagLong) {
                tags.put(key, new LongTag(((NBTTagLong) base).e()));
                continue;
            }
            if (base instanceof NBTTagShort) {
                tags.put(key, new ShortTag(((NBTTagShort) base).g()));
                continue;
            }
            if (base instanceof NBTTagString) {
                tags.put(key, new StringTag(base.e_()));
                continue;
            }
            ...
            comp.a("test", nbase);
            ListTagBuilder ltb = new ListTagBuilder(CompoundTag_v1_19_R1.fromNMSTag(comp).getValue().get("test").getClass());
            for (int i = 0; i < list.size(); ++i) {
                NBTBase nbase2 = list.k(i);
                NBTTagCompound comp2 = new NBTTagCompound();
                comp2.a("test", nbase2);
                ltb.add(CompoundTag_v1_19_R1.fromNMSTag(comp2).getValue().get("test"));
            }
            tags.put(key, ltb.build());
        }
        return new CompoundTag_v1_19_R1(tags);
    }
    ...
    public NBTTagCompound toNMSTag() {
        ...
        for (Map.Entry entry : this.value.entrySet()) {
            ...
            if (entry.getValue() instanceof CompoundTag) {
                tag.a((String) entry.getKey(), ((CompoundTag_v1_19_R1) entry.getValue()).toNMSTag());
                continue;
            }
            ...
            if (entry.getValue() instanceof ListTag) {
                ...
                while (iterator.hasNext()) {
                    ...
                    btags.put("test", btag);
                    CompoundTag_v1_19_R1 comp = new CompoundTag_v1_19_R1(btags);
    ...
    public NBTTagCompound toNMSTag(DropMetadata meta) {
        ...
        for (Map.Entry entry : this.value.entrySet()) {
            ...
            if (entry.getValue() instanceof CompoundTag) {
                tag.a((String) entry.getKey(), ((CompoundTag_v1_19_R1) entry.getValue()).toNMSTag(meta));
                continue;
            }
            if (entry.getValue() instanceof DoubleTag) {
                tag.a((String) entry.getKey(), ((DoubleTag) entry.getValue()).getValue().doubleValue());
