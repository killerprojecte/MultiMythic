...
import io.lumine.xikage.mythicmobs.volatilecode.v1_19_R1.ai.CustomAIAdapter;
...
public class VolatileAIHandler_v1_19_R1 implements VolatileAIHandler {
    ...
    public VolatileAIHandler_v1_19_R1(VolatileCodeHandler handler) {
        ...
        for (Class<?> clazz : new VersionCompliantReflections("io.lumine.xikage.mythicmobs.volatilecode.v1_19_R1.ai.goals")
        ...
        for (Class<?> clazz : new VersionCompliantReflections("io.lumine.xikage.mythicmobs.volatilecode.v1_19_R1.ai.targeters")
                .getTypesAnnotatedWith(MythicAIGoal.class)) {
            try {
                String name = clazz.getAnnotation(MythicAIGoal.class).name();
                String[] aliases = clazz.getAnnotation(MythicAIGoal.class).aliases();
    ...
    public void navigateToLocation(AbstractEntity entity, AbstractLocation destination, double maxDistance) {
        if (entity.isLiving()) {
            ...
            e.D().a(destination.getX(), destination.getY(), destination.getZ(), maxDistance);
        }
    }
