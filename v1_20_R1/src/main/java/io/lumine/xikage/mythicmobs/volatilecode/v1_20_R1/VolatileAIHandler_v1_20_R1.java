...
import io.lumine.xikage.mythicmobs.volatilecode.v1_20_R1.ai.CustomAIAdapter;
...
public class VolatileAIHandler_v1_20_R1 implements VolatileAIHandler {
    ...
    private final Reflector<EntityInsentient> refEntityInsentient = new Reflector<EntityInsentient>(EntityInsentient.class, "bN", "bO", "bP");
    ...
    public VolatileAIHandler_v1_20_R1(VolatileCodeHandler handler) {
        ...
        for (Class<?> clazz : new VersionCompliantReflections("io.lumine.xikage.mythicmobs.volatilecode.v1_20_R1.ai.goals")
        ...
        for (Class<?> clazz : new VersionCompliantReflections("io.lumine.xikage.mythicmobs.volatilecode.v1_20_R1.ai.targeters")
                .getTypesAnnotatedWith(MythicAIGoal.class)) {
            try {
                String name = clazz.getAnnotation(MythicAIGoal.class).name();
                String[] aliases = clazz.getAnnotation(MythicAIGoal.class).aliases();
    ...
    public void navigateToLocation(AbstractEntity entity, AbstractLocation destination, double maxDistance) {
        if (entity.isLiving()) {
            ...
            e.J().a(destination.getX(), destination.getY(), destination.getZ(), maxDistance);
        }
    }
