...
public class VolatileWorldHandler_v1_20_R1 implements VolatileWorldHandler {
    public Thread resetServerThread;

    public VolatileWorldHandler_v1_20_R1(VolatileCodeHandler handler) {
    }
    ...
    public void doBlockTossEffect(AbstractLocation target, Material material, AbstractVector velocity, int duration, boolean hideSourceBlock) {
        ...
        try {
            Constructor<EntityFallingBlock> ref = (Constructor<EntityFallingBlock>) Reflector.getConstructor(EntityFallingBlock.class, net.minecraft.world.level.World.class, Double.class, Double.class, Double.class, IBlockData.class);
            ...
            PacketPlayOutEntityVelocity packetV = new PacketPlayOutEntityVelocity(block.af(), new Vec3D(velocity.getX(), velocity.getY(), velocity.getZ()));
            for (EntityPlayer p : nmsWorld.a(sp -> sp.dI().equals(nmsWorld))) {
                if (hideSourceBlock) {
                ...
                Schedulers.async().runLater(() -> {
                    PacketPlayOutEntityDestroy packet2 = new PacketPlayOutEntityDestroy(block.af());
                    p.c.a(packet2);
                    if (hideSourceBlock) {
