...
import io.lumine.xikage.mythicmobs.volatilecode.v1_20_R1.*;
...
public class VolatileCodeEnabled_v1_20_R1 implements VolatileCodeHandler {
    ...
    private final VolatileAIHandler AIHandler = new VolatileAIHandler_v1_20_R1(this);
    private final VolatileBlockHandler blockHandler = new VolatileBlockHandler_v1_20_R1(this);
    private final VolatileEntityHandler entityHandler = new VolatileEntityHandler_v1_20_R1(this);
    private final VolatileItemHandler itemHandler = new VolatileItemHandler_v1_20_R1(this);
    private final VolatileWorldHandler worldHandler = new VolatileWorldHandler_v1_20_R1(this);

    @Override
    public CompoundTag createCompoundTag(Map<String, Tag> value) {
        return new CompoundTag_v1_20_R1(value);
    }
    ...
    public void doDamage(ActiveMob mob, AbstractEntity t, float amount) {
        ...
        target.getHandle().a(caster.getHandle().dI().ag().b(caster.getHandle()), amount);
    }

    @Override
    public double getAbsorptionHearts(LivingEntity entity) {
        return ((CraftLivingEntity) entity).getHandle().ff();
    }

    @Override
    public void saveSkinData(Player player, String name) {
        GameProfile profile = ((CraftPlayer) player).getHandle().fM();
    ...
    public void lookAtLocation(AbstractEntity entity, AbstractLocation to, boolean headOnly, boolean immediate) {
        ...
        if (immediate || headOnly || BAD_CONTROLLER_LOOK.contains(handle.getBukkitEntity().getType()) || !(handle instanceof EntityInsentient)) {
            ...
            ((EntityInsentient) handle).E().a(to.getX(), to.getY(), to.getZ());
        }
    }
