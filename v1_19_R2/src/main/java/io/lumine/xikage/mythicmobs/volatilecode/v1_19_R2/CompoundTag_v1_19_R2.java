...
public class CompoundTag_v1_19_R2 extends CompoundTag {
    public CompoundTag_v1_19_R2(Map<String, Tag> value) {
        super(value);
    }
    ...
    public static CompoundTag fromNMSTag(NBTTagCompound tag) {
        ...
        for (String key : tag.e()) {
            ...
            if (base instanceof NBTTagCompound) {
                tags.put(key, CompoundTag_v1_19_R2.fromNMSTag((NBTTagCompound) base));
                continue;
            }
            ...
            if (base instanceof NBTTagString) {
                tags.put(key, new StringTag(base.f_()));
                continue;
            }
            ...
            comp.a("test", nbase);
            ListTagBuilder ltb = new ListTagBuilder(CompoundTag_v1_19_R2.fromNMSTag(comp).getValue().get("test").getClass());
            for (int i = 0; i < list.size(); ++i) {
                NBTBase nbase2 = list.k(i);
                NBTTagCompound comp2 = new NBTTagCompound();
                comp2.a("test", nbase2);
                ltb.add(CompoundTag_v1_19_R2.fromNMSTag(comp2).getValue().get("test"));
            }
            tags.put(key, ltb.build());
        }
        return new CompoundTag_v1_19_R2(tags);
    }
    ...
    public NBTTagCompound toNMSTag() {
        ...
        for (Map.Entry entry : this.value.entrySet()) {
            ...
            if (entry.getValue() instanceof CompoundTag) {
                tag.a((String) entry.getKey(), ((CompoundTag_v1_19_R2) entry.getValue()).toNMSTag());
                continue;
            }
            ...
            if (entry.getValue() instanceof ListTag) {
                ...
                for (Tag btag : tags) {
                    ...
                    btags.put("test", btag);
                    CompoundTag_v1_19_R2 comp = new CompoundTag_v1_19_R2(btags);
    ...
    public NBTTagCompound toNMSTag(DropMetadata meta) {
        ...
        for (Map.Entry entry : this.value.entrySet()) {
            ...
            if (entry.getValue() instanceof CompoundTag) {
                tag.a((String) entry.getKey(), ((CompoundTag_v1_19_R2) entry.getValue()).toNMSTag(meta));
                continue;
            }
            if (entry.getValue() instanceof DoubleTag) {
                tag.a((String) entry.getKey(), ((DoubleTag) entry.getValue()).getValue().doubleValue());
