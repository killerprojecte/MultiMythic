...
import io.lumine.xikage.mythicmobs.volatilecode.v1_19_R3.*;
...
public class VolatileCodeEnabled_v1_19_R3 implements VolatileCodeHandler {
    ...
    private final VolatileAIHandler AIHandler = new VolatileAIHandler_v1_19_R3(this);
    private final VolatileBlockHandler blockHandler = new VolatileBlockHandler_v1_19_R3(this);
    private final VolatileEntityHandler entityHandler = new VolatileEntityHandler_v1_19_R3(this);
    private final VolatileItemHandler itemHandler = new VolatileItemHandler_v1_19_R3(this);
    private final VolatileWorldHandler worldHandler = new VolatileWorldHandler_v1_19_R3(this);

    @Override
    public CompoundTag createCompoundTag(Map<String, Tag> value) {
        return new CompoundTag_v1_19_R3(value);
    }
    ...
    public void doDamage(ActiveMob mob, AbstractEntity t, float amount) {
        ...
        target.getHandle().a(caster.getHandle().Y().af().b(caster.getHandle()), amount);
    }

    @Override
    public double getAbsorptionHearts(LivingEntity entity) {
        return ((CraftLivingEntity) entity).getHandle().fb();
    }

    @Override
    public void saveSkinData(Player player, String name) {
        GameProfile profile = ((CraftPlayer) player).getHandle().fI();
    ...
    public void lookAtLocation(AbstractEntity entity, AbstractLocation to, boolean headOnly, boolean immediate) {
        ...
        if (immediate || headOnly || BAD_CONTROLLER_LOOK.contains(handle.getBukkitEntity().getType()) || !(handle instanceof EntityInsentient)) {
            ...
            ((EntityInsentient) handle).C().a(to.getX(), to.getY(), to.getZ());
        }
    }
