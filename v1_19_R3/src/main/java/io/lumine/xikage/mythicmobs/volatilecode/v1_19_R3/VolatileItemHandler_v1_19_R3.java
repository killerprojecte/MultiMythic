...
public class VolatileItemHandler_v1_19_R3 implements VolatileItemHandler {
    private final Reflector refItemStack = new Reflector(CraftItemStack.class, "handle");

    public VolatileItemHandler_v1_19_R3(VolatileCodeHandler handler) {
    }
    ...
    public ItemStack addNBTData(ItemStack itemStack, String key, Tag value) {
        ...
        CompoundTag compound = CompoundTag_v1_19_R3.fromNMSTag(tag).createBuilder().put(key, value).build();
        nmsItemStack.c(((CompoundTag_v1_19_R3) compound).toNMSTag());
        return itemStack;
    }
    ...
    public CompoundTag getNBTData(ItemStack itemStack) {
        ...
        if (nmsItemStack != null && nmsItemStack.t()) {
            return CompoundTag_v1_19_R3.fromNMSTag(nmsItemStack.u());
        }
        return new CompoundTag_v1_19_R3(new HashMap<String, Tag>());
    }

    @Override
    public ItemStack setNBTData(ItemStack itemStack, CompoundTag compoundTag) {
        net.minecraft.world.item.ItemStack nmsItemStack = CraftItemStack.asNMSCopy(itemStack);
        nmsItemStack.c(((CompoundTag_v1_19_R3) compoundTag).toNMSTag());
    ...
    public ItemStack setNBTData(ItemStack itemStack, CompoundTag compoundTag, DropMetadata meta) {
        net.minecraft.world.item.ItemStack nmsItemStack = CraftItemStack.asNMSCopy(itemStack);
        nmsItemStack.c(((CompoundTag_v1_19_R3) compoundTag).toNMSTag(meta));
        return CraftItemStack.asBukkitCopy(nmsItemStack);
    }
